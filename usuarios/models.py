...
class User(AbstractBaseUser, PermissionsMixin):
    ...
    conf_not_email = models.BooleanField(
        default=False, verbose_name="Receber notificações por e-mail"
    )

    conf_not_push = models.BooleanField(
        default=False, verbose_name="Receber notificações push"
    )

    conf_not_newdenun = models.BooleanField(
        default=False, verbose_name="Notificar novas denúncias"
    )

    date_created = models.DateTimeField(
        default=now, verbose_name="Data de criação", editable=False
    )
